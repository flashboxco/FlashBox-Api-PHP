...
class FlashBoxApiHandler
{
    ...
    public function __call($name, $arguments)
    {
        ...
        throw new FlashBoxApiException('FlashBox API: This Function Does Not Exist!');
    }
    ...
    private static function getCurlOptions($endPoint = '', $method = 'GET', $postFields = null)
    {
        ...
        if (!extension_loaded('openssl')) {
            throw new FlashBoxApiException('FlashBox API Needs The Open SSL PHP Extension! please enable it on your server.');
        }
        /*
         * Get ACCESS-TOKEN
         */
        ...
        if (!$accessToken) {
            throw new FlashBoxApiException('Invalid ACCESS-TOKEN! 
            ...
            Put it in: vendor/flashbox/flashbox-api-php/src/Config/Configs.php : TOKEN const 
            ');
        }
    ...
    private static function getApiResponse($curlObject)
    {
        ...
        if ($err) {
            throw new FlashBoxApiException($err);
        } else {
    ...
    public static function setEndpoint($env = 'production', $endpoint = null)
    {
        ...
        if ($env == 'production') {
            ...
            if (!is_array($endpoint) || !isset($endpoint['url']) || !isset($endpoint['api_url']) || !isset($endpoint['tracking_url'])) {
                throw new FlashBoxApiException('Endpoint is not correct');
            }
        } else {
    ...
    public static function getLocationSuggestion($locationName, $latlng)
    {
        ...
        $locationName = FlashBoxValidator::sanitize($locationName);
        if (empty($locationName)) {
            throw new FlashBoxApiException('Location Name can not be empty!');
        }
    ...
    public static function getOrderDetail($orderID)
    {
        ...
        $orderID = FlashBoxValidator::sanitize($orderID);
        if (!filter_var($orderID, FILTER_VALIDATE_INT)) {
            throw new FlashBoxApiException('OrderID must be integer!');
        }
